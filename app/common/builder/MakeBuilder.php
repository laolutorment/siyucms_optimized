...
class MakeBuilder
{
    ...
    public function getListColumns(string $tableName = '')
    {
        ...
        foreach ($fields as &$field) {
            ...
            // 位置编号ZTX-009，重新调整字段属性设置，把列表的字段属性和表单的字段属性分开。
            // 当table_col_type为空或者无时，等于type字段的值
            $table_col_type = $field['table_col_type'] ?: $field['type'];
            // 位置编号ZTX-009，重新调整字段属性设置，把列表的字段属性和表单的字段属性分开。
            ...
            // 添加到返回数组中
            // 位置编号ZTX-009，重新调整字段属性设置，把列表的字段属性和表单的字段属性分开。
            $columns[] = [$field['field'], $field['name'],$table_col_type, $default, $param, $class, $sortable,$field['table_col_align'],$field['table_col_visible'],$field['table_col_formatter']];
            // 原代码：$columns[] = [$field['field'], $field['name'], $field['type'], $default, $param, $class, $sortable];
            // 位置编号ZTX-009，重新调整字段属性设置，把列表的字段属性和表单的字段属性分开。
    ...
    public function getAddColumns(string $tableName = '', array $info = [])
    {
        ...
        foreach ($fields as &$field) {
            ...
            if ($field['type'] == 'text') {
                ...
                $field['setup']['default'] = is_int($field['setup']['default'])&&$field['setup']['default']!=0 ? date($format, $field['setup']['default']) : $field['setup']['default'];//ZTX-添加条件判断&&$field['setup']['default']!=0，修复表单时间显示的问题
                $columns[]                 = [
                    $field['type'],                // 类型
                    $field['field'],               // 字段名称
                    $field['name'],                // 字段别名
                ...
                $columns[] = [
                    ...
                    $field['tips'],                       // 提示信息
                    $field['setup']['default'],           // 默认值
                    $field['setup']['height'] ?? '500',
                    $field['setup']['extra_attr'] ?? '',  // 额外属性
                    $field['setup']['extra_class'] ?? '', // 额外CSS
                    $field['required'],                   // 是否必填
                    $field['setup']['mode'] ?? '', // 代码模式
                    $field['setup']['theme'] ?? '', // 编辑器主题
                    ...
                    for ($i = 0; $i < count($formData[$k]); $i++) {
                        if ($formData[$k][$i]) {
                            ...
                            $images[$i]['title'] = $formData[$k . '_title'][$i];
                             // ZTX-增加缩略图显示
                            if($fieldsNew[$k]['type'] == 'images'){
                                $images[$i]['small'] = $formData[$k . '_small'][$i]??'';  
                            }
                             // ZTX-增加缩略图显示
                        }
                    }
            ...
            foreach ($fieldsNew as $kk => $vv) {
                ...
                if (array_key_exists($kk, $v) && $kk != 'create_time' && $kk != 'update_time') {
                    ...
                    if ($vv['type'] == 'date' || $vv['type'] == 'time' || $vv['type'] == 'datetime') {
                        ...
                        $data[$k][$kk] = $data[$k][$kk] === 0 ||  $data[$k][$kk] === '0' ||  $data[$k][$kk] === '' ? '' : date($format, $data[$k][$kk]);//ZTX-修复1970年以前日期的bug
                    }
                }
            }
        }
