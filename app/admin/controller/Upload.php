...
class Upload extends Base
{
    ...
    private function bigUpload()
    {
        ...
        // 设置上传目录
		//ZTX-005目录使用网址传递参数
        $uploadDir = public_path() . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . $newdir;
        // 上传完后清空临时目录
...
//   原代码：         @mkdir($uploadDir);
        ...
        $oldName = $fileName;
       //ZTX-004  $fileName = iconv('UTF-8', 'gb2312', $fileName);
    ...
    //ZTX-005修改命名规则
        //※源代码 $fileName = md5(time() . $pathInfo['basename']) . '.' . $pathInfo['extension'];
        $fileName =$pathInfo['filename'].'_'. md5(time() . 'ZTX') . '.' . $pathInfo['extension'];
        ...
        if ($done) {
            ...
            if($file_type=='img'){
                ...
                if(filesize($smallimage)){
                    ...
                    $modelFM::create([
                        ...
                        'module' => $newdir,
                        'describe' =>$newdir.'_small',
                        'file_type' => $file_type.'/'.$pathInfo['extension'],
                        'file_size' =>round(filesize($smallimage)/1024)
			...
			$file_type=$file_type."/".substr(strrchr($_FILES["file"]['name'], '.'), 1);
			$modelFM::create([
